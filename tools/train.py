...
class Batch:
    """画像とトークン列。**長さがばらばら**なので、1 件ずつ回して損失を足す。

    まとめて行列にすればもっと速いが、C++ 側と「同じ量」を計算していることを
    確かめやすいほうを取る（速さが要るときは --device cuda で足りる）。
    """
    ...
    for step in range(a.steps):
        ...
        for _ in range(a.batch):
            ...
            if path not in cache:
                cache[path] = load_gray(path, c.H, c.W)
            ids = tok.encode(latex, bos=True, eos=True)
            if len(ids) < 2 or len(ids) > c.max_len:
                continue
            img = torch.from_numpy(cache[path]).reshape(1, 1, c.H, c.W).to(dev)
            logits = m(img, ids[:-1])
        ...
        if a.log_every > 0 and (step % a.log_every == 0 or step == a.steps - 1):
            print("step %d: loss %.4f |g| %.2f" % (step, l, gn))
            sys.stdout.flush()
    print("loss %.4f -> %.4f（%d step、%.1f 秒）" % (first, last, a.steps, time.time() - t0))
    if a.export:
        os.makedirs(os.path.dirname(a.export) or ".", exist_ok=True)
        M.save(m.cpu(), a.export)
