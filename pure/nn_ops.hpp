// ふつうの層に要る微分可能な演算 — ReLU、非対称パディング、global average pool、
// dense linear（matmul + bias）、行ごとの L2 正規化。
//
// 出どころ: 姉妹リポ facenet_cpp / depth_anything_cpp の face_ops.hpp。**中身はそのまま**。
// backward の登録の作法は autograd.hpp と同じ。
#pragma once
